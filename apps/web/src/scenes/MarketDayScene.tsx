...
function PriceCrystals({ count, position }: { count: number; position: readonly [number, number, number] }) {
  const rows: number[] = [];
  let left = count;
  while (left > 0) {
      ...
      {rows.map((rowCount, rowIndex) => (
        <group key={rowIndex} position={[0, rowIndex * 0.24, 0]}>
          {Array.from({ length: rowCount }, (_, i) => (
            <mesh key={i} position={[(i - (rowCount - 1) / 2) * 0.22, 0, 0]} castShadow>
              <icosahedronGeometry args={[0.08, 0]} />
              <meshPhysicalMaterial color="#7fe3c9" emissive="#0f3b30" emissiveIntensity={0.25} roughness={0.2} clearcoat={0.8} />
            </mesh>
          ))}
        </group>
      ))}
...
// The beam's tilt is driven directly by remaining(state) — never by a
// physics simulation, so it can never still be settling when the
// total becomes exact (I6).
function BalanceBeam({ tiltRef }: { tiltRef: React.MutableRefObject<number> }) {
  const beamRef = useRef<THREE.Mesh>(null);
  useFrame((_, delta) => {
    if (!beamRef.current) return;
    beamRef.current.rotation.z = THREE.MathUtils.damp(beamRef.current.rotation.z, tiltRef.current, 6, delta);
  });
      ...
      <group position={[0, 0.42, 0]}>
        <mesh ref={beamRef} castShadow>
          ...
          <mesh position={[-0.62, -0.06, 0]}>
            ...
            <meshStandardMaterial color="#7fe3c9" roughness={0.4} />
          </mesh>
          <mesh position={[0.62, -0.06, 0]}>
            <sphereGeometry args={[0.06, 12, 12]} />
            <meshStandardMaterial color="#ffb570" roughness={0.4} />
          </mesh>
        </mesh>
      </group>
    </group>
...
export function MarketDayScene() {
  ...
  useFrame(() => {
    const MAX_TILT = 0.35;
    tiltRef.current = THREE.MathUtils.clamp(remaining / order.price, -1, 1) * MAX_TILT;
  });
  ...
  return (
    <group>
      ...
      <PriceCrystals count={order.price} position={[0, COUNTER_Y + 0.55, ITEM_Z]} />

      <BalanceBeam tiltRef={tiltRef} />
